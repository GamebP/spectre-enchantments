...
#define TRANSPARENCY_COLOR RGB(0, 254, 0)
    ...
    inline int amain() {
        ...
        if (!hwnd) {
            cout << _("Window HiJacking failed") << endl;
            return EXIT_FAILURE;
        }
    ...
    inline HWND HiJackNotepadWindow() {
        ...
        system(_("start winver")); // Start notepad, and not as child process, so easy :)
        ...
        Addresses::Width = GetSystemMetrics(SM_CXSCREEN);
        Addresses::Height = GetSystemMetrics(SM_CYSCREEN);
        return hwnd;
    }
