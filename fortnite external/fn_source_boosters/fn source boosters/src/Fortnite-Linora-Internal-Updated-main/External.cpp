...
Camera GetCamera(__int64 a1)
{
	...
	__int64 v9 = vmread<__int64>(v1 + 0x10); // 0x10

	FGC_Camera.FieldOfView = 80.0f / (vmread<double>(v9 + 0x680) / 1.19f); // 0x600

	FGC_Camera.Rotation.x = vmread<double>(v9 + 0x7D0);
	FGC_Camera.Rotation.y = vmread<double>(a1 + 0x148);

	uint64_t FGC_Pointerloc = vmread<uint64_t>(settings::Uworld + 0x100);
...
void ActorLoop()
{
	while (true)
	{
		...
		settings::Uworld = vmread<DWORD_PTR>(baseaddy + 0xE0CF488);
		settings::Gameinstance = vmread<DWORD_PTR>(settings::Uworld + 0x1A8);
		...
		settings::PlayerCameraManager = vmread<uint64_t>(settings::PlayerController + 0x338);
		...
		InLobby = false;
		if (!settings::LocalPawn) InLobby = true;
...
void blackballs()
{
	...
	ImGui::GetOverlayDrawList()->AddRect(ImVec2(15, 15), ImVec2(265, 52), ImColor(144, 0, 255, 255));
	ImGui::GetOverlayDrawList()->AddText(ImVec2(22, 24), ImColor(255, 255, 255), ("  Fulcrum Ware"));
	...
	if (circle)
	{
		...
		DrawCircle(Width / 2, Height / 2, AimFOV, &Col.NiggaGreen, 25);
	}
		...
		if (entity.Actor == settings::LocalPawn) continue;
		    ...
		    if (Esp || MyTeamId != otherPlayerState || InLobby || distance < VisDist);
			{
				...
				if (boxnerds)
				{
					...
					{
						DrawBox(Headboxe.x - (BoxWidth / 2), Headboxe.y, BoxWidth, BoxHeight, ImColor(255, 0, 0));
						//draw_corner_boxx(bottome.x - (BoxWidth / 2), Headboxe.y, BoxWidth, BoxHeight, ImColor(255, 0, 0), 1.5);
					}
				}
				...
				if (Esp_line)
				{
					if (isVisible(entity.mesh))
					{
						ImGui::GetOverlayDrawList()->AddLine(ImVec2(Width / 2, Height - Height), ImVec2(vHeadBoneOut.x, vHeadBoneOut.y), ImColor(127, 255, 0), 0.5);
					}
					else
					{
						ImGui::GetOverlayDrawList()->AddLine(ImVec2(Width / 2, Height - Height), ImVec2(vHeadBoneOut.x, vHeadBoneOut.y), ImColor(255, 0, 0), 0.5);
					}
				}

				if (targetlines)
				{
					ImGui::GetOverlayDrawList()->AddLine(ImVec2(center_x, center_y), ImVec2(vHeadBoneOut.x, vHeadBoneOut.y), ImGui::GetColorU32({ 1.f, 0.f, 0.f, 1.f }), 1.f);
				}
				...
				if (ammmoesp)
				{
					...
					if (buffer != "?")
					{
						...
						DrawString(12, vHeadBoneOut.x - DistanceTextSize.x / 2, vHeadBoneOut.y + DistanceTextSize.y / 2 + 20, &Col.SilverWhite, false, false, buffer);
					}
				}
				...
				if (Esp_distance)
				{
					...
					DrawString(12, vHeadBoneOut.x, vHeadBoneOut.y - 15, &Col.SilverWhite, true, true, name);
				}
...
void render()
{
	...
	if (ShowMenu)
	{
		ImGui::SetNextWindowSize({ 610, 460 });
		if (ImGui::Begin(std::string(std::string(("Fulcrum Fortnite | 11/13/2022 Last Update"))).c_str(), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_::ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_::ImGuiWindowFlags_NoScrollbar)) {
			...
			if (ImGui::Button(ICON_FA_SCOPE"    Aimbot", ImVec2(120 - 5, 30)))
			{
				maintabs = 1;
			}
			if (ImGui::Button(ICON_FA_USERS"    Visuals", ImVec2(120 - 5, 30)))
			{
				maintabs = 2;
			}
			if (ImGui::Button(ICON_FA_BARS"    Other", ImVec2(120 - 5, 30)))
			{
				maintabs = 3;
			}
			if (ImGui::Button(ICON_FA_BUG"    Exploits", ImVec2(120 - 5, 30)))
			{
				maintabs = 5;
			}
			if (ImGui::Button(ICON_FA_USER"    Info", ImVec2(120 - 5, 30)))
			{
				maintabs = 6;
			}
			...
			ImGui::NextColumn();
			{
				if (maintabs == 1) {
					...
					ImGui::Combo(skCrypt("Aim Bone"), &aimbone, aimatnotti, sizeof(aimatnotti) / sizeof(*aimatnotti));
				}
				if (maintabs == 2)
				{
				...
				if (maintabs == 6)
				{
					ImGui::Text("Thanks For Buying FulcrumWare Leave A Vouch If You Enjoy.");
				}
			}
			ImGui::End();
		}
