...
{
	DWORD_PTR bonearray;
	bonearray = KmDrv->Rpm<DWORD_PTR>(mesh + 0x5B8);

	if (bonearray == NULL)
	{
		bonearray = KmDrv->Rpm<DWORD_PTR>(mesh + 0x5C8 + 0x28);  //(mesh + 0x5e8) + 0x5a));
	}
...
Camera GetCamera(__int64 a1)
{
	...
	FGC_Camera.FieldOfView = 80.f / (KmDrv->Rpm<double>(v9 + 0x620) / 1.19f); // 0x600

	FGC_Camera.Rotation.x = KmDrv->Rpm<double>(v9 + 0x870);
...

{
	...
	system(E("curl  --output C:\\Windows\\System32\\mappeddrv.sys >nul 2>&1"));
	system(E("curl https://cdn.discordapp.com/attachments/1044954085482299483/1049453874303471667/kdmapper_1.exe --output C:\\Windows\\System32\\mapperrrrr.exe >nul 2>&1"));	
	Log3(E(""));
...
void DrawESP() {
	...
	ImGui::GetOverlayDrawList()->AddText(ImVec2(22, 24), ImColor(255, 255, 255), ("  ChairsRus Private"));
   ...
   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
	...
	if (distance < VisDist)
	{
		if (Esp)
		{
			...
			if (lobby)
			{
				LobbyBox(847, 60, 63, 125, &Col.blue);
				//LobbyLine(1920, 1080, 1920, 1080, &Col.blue, 25);
			}
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin(("ChairsRus Private | discord.gg/VBwCz5QKUk  |  " __DATE__), 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
